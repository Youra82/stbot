...
class TqdmCallback(Callback):
    def __init__(self, pbar): super().__init__(); self.pbar = pbar
...
class StochRSIOptimizationProblem(Problem):
    def __init__(self, trend_filter_enabled, sideways_filter_enabled, **kwargs):
        ...
        super().__init__(n_var=9, n_obj=2, n_constr=0,
                         xl=[5, 5, 2, 2, 5, 0.1, 5, 1.0, 2],
                         xu=[50, 50, 10, 10, 50, 1.0, 50, 5.0, 15], **kwargs)

    def _evaluate(self, x, out, *args, **kwargs):
        results = []
        for individual in x:
            params = {
                'stoch_rsi_period': int(round(individual[0])), 'stoch_period': int(round(individual[1])),
                'stoch_k': int(round(individual[2])), 'stoch_d': int(round(individual[3])),
                'swing_lookback': int(round(individual[4])), 'sl_buffer_pct': round(individual[5], 2),
                'base_leverage': int(round(individual[6])), 'target_atr_pct': round(individual[7], 2),
                ...
                'sideways_filter': { 'enabled': self.sideways_filter_enabled, 'lookback': 50, 'max_crosses': int(round(individual[8])) }
            }
            ...
            with Pool(n_procs) as pool:
                ...
                for i in sorted(valid_indices, key=lambda i: res.F[i][0])[:5]:
                    ...
                    param_dict = {
                        ...
                        'params': {
                            'stoch_rsi_period': int(round(params[0])), 'stoch_period': int(round(params[1])),
                            'stoch_k': int(round(params[2])), 'stoch_d': int(round(params[3])),
                            'swing_lookback': int(round(params[4])), 'sl_buffer_pct': round(params[5], 2),
                            'base_leverage': int(round(params[6])), 'target_atr_pct': round(params[7], 2),
                            'trend_filter': { 'enabled': is_trend_filter_enabled, 'period': 200 },
                            'sideways_filter': { 'enabled': is_sideways_filter_enabled, 'lookback': 50, 'max_crosses': int(round(params[8])) }
                        }
                    }

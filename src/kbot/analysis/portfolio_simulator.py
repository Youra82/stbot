# /root/kbot/src/kbot/analysis/portfolio_simulator.py
import pandas as pd
import numpy as np
...
def run_portfolio_simulation(start_capital, strategies_data, start_date, end_date):
    """
    Führt eine chronologische Portfolio-Simulation mit mehreren KBot-Strategien durch.
    (Strategie: StochRSI)
    """
    print("\n--- Starte Portfolio-Simulation (KBot StochRSI)... ---")
    ...
    for key, strat in tqdm(strategies_data.items(), desc="Verarbeite Strategien"):
        try:
            ...
            params = strat.get('smc_params', {}) # Rückwärtskompatibler Key
            ...
            # 1b. StochRSI Engine
            engine = StochRSIEngine(settings=params)

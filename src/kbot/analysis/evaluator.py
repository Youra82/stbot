# src/kbot/analysis/evaluator.py
import pandas as pd
import numpy as np
import ta
import sys
...
def evaluate_dataset(data: pd.DataFrame, timeframe: str):
    ...
    basierend auf Volatilität, Marktphasen und Signal-Dichte (StochRSI).
    """
    ...
    # --- Metrik 2: Handelbarkeit / Signal-Dichte (max. 4 Punkte) ---
    try:
        engine = StochRSIEngine(settings={'rsi_period':14, 'stoch_rsi_period':14, 'stoch_k':3, 'stoch_d':3})
        df = engine.process_dataframe(data.copy())
        event_count = df['sr_signal'].abs().sum()
        event_density = (event_count / len(data)) * 1000 if len(data) > 0 else 0
    except Exception:
        event_density = 0

    if event_density < 5: score2 = 0
    elif event_density < 15: score2 = 1
    elif event_density < 40: score2 = 2
    elif event_density < 100: score2 = 3
    else: score2 = 4
    just2 = f"- Handelbarkeit ({score2}/4): {'Exzellent' if score2==4 else 'Gut' if score2==3 else 'Mäßig' if score2==2 else 'Gering' if score2==1 else 'Sehr Gering'}. {event_density:.1f} Signals/1000 Kerzen."

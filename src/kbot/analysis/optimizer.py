# /root/kbot/src/kbot/analysis/optimizer.py
import os
import sys
import json
import optuna
...
def objective(trial):
    # --- StochRSI Strategie Parameter (kbot) ---
    strategy_params = {
        'rsi_period': trial.suggest_int('rsi_period', 7, 21),
        'stoch_rsi_period': trial.suggest_int('stoch_rsi_period', 7, 21),
        'stoch_k': trial.suggest_int('stoch_k', 1, 6),
        'stoch_d': trial.suggest_int('stoch_d', 1, 6),
        'stoch_rsi_low': trial.suggest_int('stoch_rsi_low', 5, 40),
        'stoch_rsi_high': trial.suggest_int('stoch_rsi_high', 60, 95),
    ...
    # --- Risiko Parameter ---
    risk_params = {
        'risk_reward_ratio': trial.suggest_float('risk_reward_ratio', 1.2, 5.0),
    ...
    if OPTIM_MODE == "strict" and (
        ...
        pnl < MIN_PNL_CONSTRAINT or trades < 20):
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Parameter-Optimierung für KBot (StochRSI)")
    ...
    for task_idx, task in enumerate(TASKS, 1):
        ...
        DB_FILE = os.path.join(PROJECT_ROOT, 'artifacts', 'db', 'optuna_studies_kbot.db')
        os.makedirs(os.path.dirname(DB_FILE), exist_ok=True)
        STORAGE_URL = f"sqlite:///{DB_FILE}?timeout=60"
        study_name = f"stochrsi_{create_safe_filename(symbol, timeframe)}{CONFIG_SUFFIX}_{OPTIM_MODE}"
        ...
        strategy_config = {
            ...
            'stoch_rsi_period': best_params['stoch_rsi_period'],
            'stoch_k': best_params['stoch_k'],
            'stoch_d': best_params['stoch_d'],
            'stoch_rsi_low': best_params['stoch_rsi_low'],
            'stoch_rsi_high': best_params['stoch_rsi_high']
        }

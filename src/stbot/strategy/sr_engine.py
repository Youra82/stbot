...
class SREngine:
    ...
    def process_dataframe(self, df: pd.DataFrame):
        ...
        # Ein Pivot ist, wenn der Wert gleich dem Rolling Max/Min ist
        is_pivot_high = (src1 == max_roll)
        is_pivot_low = (src2 == min_roll)
        
        # JETZT verschieben wir das Wissen um 'prd' in die Zukunft.
        # Fix für Pandas FutureWarning: Wir nutzen fillna(False) und casten explizit zu bool.
        ...
        # Die Werte der Pivots holen (die Werte von vor 'prd' Kerzen)
        pivot_val_high = src1.shift(self.prd)
        pivot_val_low = src2.shift(self.prd)

        # 3. Dynamische Kanal-Breite berechnen
        highest_300 = df['high'].rolling(300).max()
        lowest_300 = df['low'].rolling(300).min()
        cwidths = (highest_300 - lowest_300) * self.channel_w_pct / 100
        
        # 4. Iteration durch die Kerzen (Zustandsbehaftete Logik)
        ...
        for i in range(len(df)):
            ...
            if new_val is not None and not np.isnan(new_val):
                pivotvals.insert(0, new_val) # Vorne einfügen
                if len(pivotvals) > self.maxnumpp:
                    pivotvals.pop() # Hinten entfernen
            
            # Wenn keine Pivots da sind, weiter
            if not pivotvals:
                continue
                
            current_cwidth = arr_cwidth[i]
            if current_cwidth == 0: continue

            # B. S/R Zonen berechnen (Clustering)
            temp_zones = []
            ...
            # Sortieren nach Stärke (absteigend)
            temp_zones.sort(key=lambda x: x['strength'], reverse=True)
            
            # Filtern (Overlap Check und Max SR Levels)
            final_zones = []
            for z in temp_zones:
            ...
            for z in final_zones:
                ...
                # Crossover (Resistance Broken -> Buy)
                if prev_close <= mid and curr_close > mid:
                    signals[i] = 1
                    break 
                
                # Crossunder (Support Broken -> Sell)
                if prev_close >= mid and curr_close < mid:
                    signals[i] = -1
                    break
